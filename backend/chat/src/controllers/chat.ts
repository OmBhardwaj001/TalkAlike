...
export const createNewChat = TryCatch(async(req:AuthenticatedRequest, res)=>{
    ...
    if(existingChat){
        res.json({message:'chat alreadyt exists', chatId:existingChat._id});
        return;
    }
...
export const sendMessage = TryCatch(async(req:AuthenticatedRequest, res)=>{
    ...
    if(imageFile){
     ...
     res.status(201).json({message:"saved messaege", sender:senderId});
})
...
export const getMessageByChat = TryCatch(async(req:AuthenticatedRequest,res)=>{
    ...
    try {
        ...
        console.log(error);
        res.json({messages,user:{_id:otherUserId,name:"unknown"}})
    }
})
